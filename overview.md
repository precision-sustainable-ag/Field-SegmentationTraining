...
│   ├── config.yaml                    # 6a. Root Hydra config (defaults: [projects, paths, model, datamodule, train, augment, preprocess, maskgen, inference, hydra/job_logging])
│   │
│   ├── projects/                      # 6b. “projects” group: project‐wide constants
│   │   └── field.yaml                 #     • data_subdir, num_classes, input_channels, image_size, project_name
│   │
│   ├── paths/                         # 6c. “paths” group: all filesystem paths
...
│   │
│   ├── model/                         # 6d. “model” group: architecture choices
...
│   │
│   ├── datamodule/                    # 6e. “datamodule” group: dataset + dataloader settings
│   │   └── default.yaml               #     • train/val split, batch_size, num_workers, seed, dataset dirs, transforms references
│   │
│   ├── train/                         # 6f. “train” group: optimizer, lr, epochs, scheduler, Trainer, callbacks, logger
│   │   └── default.yaml               #     • train.learning_rate, train.batch_size, train.max_epochs, optimizer._target_, scheduler._target_, trainer settings, checkpoint/early_stop/logger settings
│   │
│   ├── augment/                       # 6g. “augment” group: data augmentation settings
│   │   └── default.yaml               #     • horizontal_flip, vertical_flip, random_rotate, color_jitter, random_resized_crop, gaussian_blur, etc.
│   │
│   ├── preprocess/                    # 6h. “preprocess” group: resizing, normalization, cropping, mask remapping, train/val/test splits
...
│   │
│   ├── maskgen/                       # 6i. “maskgen” group: mask generation/refinement settings
...
│   │
│   ├── inference/                     # 6j. “inference” group: inference & evaluation settings
...
│   │
│   └── hydra/                         # 6k. Hydra overrides (job_logging, output dir, etc.)
│       └── job_logging/
...
│   └── field_segmentation/            #    (folder name matches package)
│       ├── __init__.py
│       │
│       ├── data/                      # 7a. Data + DataModule
│       │   ├── datamodule.py          #     • LightningDataModule: builds train/val/test sets, applies preprocess + augment transforms
│       │   ├── transforms.py          #     • Compose augment & preprocess pipelines based on cfg
...
│   ├── train.sh                       # 8a. Example: “bash scripts/train.sh --cfg pipeline.mode=train,model=unet,projects=field”  
│   └── infer.sh                       # 8b. Example: “bash scripts/infer.sh --cfg pipeline.mode=inference,checkpoint=…”
│
├── tests/                             # 9. Unit tests / integration tests
│   ├── test_datamodule.py             # 9a. Tests for dataloading, transforms, DataModule
...
│   └── test_maskgen.py                # 9d. (Optional) Tests for mask‐generation code (classical threshold, SAM stub, etc.)
│
